...
def main():
    ...
    # Kasam kol praeita pro visas tranzakcijas
    ...
    while tx_pool:
        ...
        mined_block = bc.mine_next_block(
            ...
            remove_from_pool=remove_from_pool,  # si funkcija atnaujins tx_pool vietoje
            miners=miners,
            block_reward=BLOCK_REWARD,
            mining_time_limit=time_limit,
        )

        # jeigu po round'o pool jau tuscias – baigiam iskart (isvengiam papildomu spausdinimu)
        if not tx_pool:
            print("✅ Transaction pool is empty. Mining completed.")
            break

        # jei niekas neiskase – padidinam laika ir kartojam
        if mined_block is None:
            time_limit *= 2
            print(f"⏱️  No block mined — increasing time limit to {time_limit}s and retrying.")
            continue

        # jei iskasam – grazinam i pradine reiksme
        time_limit = MINING_TIME_LIMIT
